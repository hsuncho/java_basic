...
public class MethodQuiz02 {
	...
	// 존재하지 않는 값이라면 -1을 리턴.
	static int indexOf(String f) {
		for(int index=0; index<foods.length;index++) {
			if(f.equals(foods[index])) {
				return index;
			}
		}
		return -1;
	...
	static boolean include(String f) {
		return indexOf(f) != -1;
	}
	...
	//매개값으로 전달한 인덱스의 값을 전다란 값으로 수정해주는 메서드
	static void modify(int idx, String f) {
		foods[idx] = f;
	}
