...
public class MainClass {
	...
	// 내부 클래스(inner class)
	// 해당 객체와 밀접한 연관이 있는 경우 클래스 내에서 클래스를 선언(private으로 선언하는 게 일반적)
...
//		System.out.println(sub3);
		...
		//곱셈 계산을 하는 계산기가 필요. (익명 클래스) //외부에서 사용 불가
		//말 그대로 이름이 없는 클래스를 즉석에서 선언
		//선언한 클래스 내에서는 주소값을 통해 객체를 사용할 수 있지만
		//외부에서는 지목할 방법이 없기 때문에 객체 생성이 불가능
		...
		//어차피 인터페이스에 추상메서드가 1개이기 때문에 이름을 따로 명시ㅏ지 않음
		//메서드 내부 로직이 단 한 줄이라면 괄호 생ㅑㄱ 가능 -> return 값으로 취급
